...
#define MIC_PACKET_SZE(frq)            (uint8_t)(((frq * 2U * 2U)/1000U) & 0xFFU), \
...
void USBD_AUDIO_Sync(USBD_HandleTypeDef *pdev, AUDIO_OffsetTypeDef offset)
{
  ...
  {
    if ((haudio->rd_ptr - haudio->wr_ptr) < AUDIO_IN_PACKET)
    {
      BufferSize += 4U;
    }
    else
    {
      if ((haudio->rd_ptr - haudio->wr_ptr) > (AUDIO_TOTAL_BUF_SIZE - AUDIO_IN_PACKET))
      {
        BufferSize -= 4U;
      }
    }
  ...
  {
    if ((haudio->wr_ptr - haudio->rd_ptr) < AUDIO_IN_PACKET)
    {
      BufferSize -= 4U;
    }
    else
    {
      if ((haudio->wr_ptr - haudio->rd_ptr) > (AUDIO_TOTAL_BUF_SIZE - AUDIO_IN_PACKET))
      {
        BufferSize += 4U;
      }
    }
...
static uint8_t USBD_AUDIO_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (epnum == AUDIO_IN_EP)
  {
    ...
    (void)USBD_LL_PrepareReceive(pdev, AUDIO_IN_EP,
...
/************************ (C) COPYRIGHT STMicroelectronics *****END OF FILE****/
>>>>>>> branch 'main' of https://github.com/mikolajguszpit/emppp.git
